        ...
        if data['success']:
            ...
            for auction in data['auctions']:
                if not auction['claimed'] and 'bin' in auction and not "Furniture" in auction["item_lore"]: # if the auction isn't a) claimed and is b) BIN
                    ...
                    if prices[index][1] > LOWEST_PRICE and prices[index][1]-prices[index][0] > LOWEST_MARGIN and auction['start']+60000 > now:
...
def main():
    ...
    if len(results): results = [[entry, prices[entry[3]][1]] for entry in results if (entry[2] > LOWEST_PRICE and prices[entry[3]][1] != float('inf') and prices[entry[3]][0] == entry[2] and prices[entry[3]][1]-prices[entry[3]][0] > LOWEST_MARGIN)]
